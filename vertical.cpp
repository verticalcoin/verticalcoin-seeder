...
#define VERTICALCOIN_SEED_NONCE  0x0539a019ca550825ULL
#define DEBUG true
...
class CNode {
  ...
  void PushVersion() {
    ...
    uint64 nLocalNonce = VERTICALCOIN_SEED_NONCE;
    int64 nLocalServices = 0;
    CAddress me(CService("0.0.0.0"));
    BeginMessage("version");
    int nBestHeight = GetRequireHeight();
    string ver = "/verticalcoin-seeder:1.3.3.7/";
  ...
  bool ProcessMessage(string strCommand, CDataStream& vRecv) {
    ...
    if (strCommand == "addr" && vAddr) {
      ...
      vRecv >> vAddrNew;
      if (DEBUG) printf("%s: got %i addresses\n", ToString(you).c_str(), (int)vAddrNew.size());
      int64 now = time(NULL);
      ...
      while (it != vAddrNew.end()) {
        CAddress &addr = *it;
        if (DEBUG) printf("%s: got address %s\n", ToString(you).c_str(), addr.ToString().c_str(), (int)(vAddr->size()));
        it++;
        if (addr.nTime <= 100000000 || addr.nTime > now + 600)
          addr.nTime = now - 5 * 86400;
        if (addr.nTime > now - 604800)
          vAddr->push_back(addr);
        if (DEBUG) printf("%s: added address %s (#%i)\n", ToString(you).c_str(), addr.ToString().c_str(), (int)(vAddr->size()));
  ...
  bool ProcessMessages() {
    ...
    do {
      ...
      vRecv >> hdr;
      if (!hdr.IsValid()) { 
        if (DEBUG) printf("%s: BAD (invalid header)\n", ToString(you).c_str());
        ban = 100000; return true;
      }
      string strCommand = hdr.GetCommand();
      unsigned int nMessageSize = hdr.nMessageSize;
      if (nMessageSize > MAX_SIZE) { 
        if (DEBUG) printf("%s: BAD (message too large)\n", ToString(you).c_str());
        ban = 100000;
        return true; 
      }
      ...
      if (ProcessMessage(strCommand, vMsg))
        return true;
      if (DEBUG) printf("%s: done processing %s\n", ToString(you).c_str(), strCommand.c_str());
    } while(1);
    return false;
  }
  ...
  bool Run() {
    ...
    while (now = time(NULL), ban == 0 && (doneAfter == 0 || doneAfter > now) && sock != INVALID_SOCKET) {
      ...
      if (nBytes > 0) {
        ...
        if (DEBUG) printf("%s: BAD (connection closed prematurely)\n", ToString(you).c_str());
        res = false;
        break;
      } else {
        if (DEBUG) printf("%s: BAD (connection error)\n", ToString(you).c_str());
        res = false;
        break;
      }
      ProcessMessages();
...
int main(void) {
  ...
  if (DEBUG) printf("ret=%s ban=%i vAddr.size()=%i\n", ret ? "good" : "bad", ban, (int)vAddr.size());
}
*/
